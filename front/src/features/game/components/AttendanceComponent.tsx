...
function AttendanceComponent({handleModal, attendance, handleReload, needReload}:any) {
  
  const [attendanceInfo, setAttendanceInfo] = useState<AttendanceInfoInterface[]>([])
  ...
  return (
    <StyledModalDiv onClick={handleModal}>
      <StyledModal onClick={(e)=> e.stopPropagation()}>
        <h1>출석체크 페이지</h1>
        <button onClick={attendance}>출석체크하기</button>
