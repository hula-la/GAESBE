...
function InModalBjConnect() {
  ...
  return <Wrapper>
    <div className="modal-title">백준 연동하기
      <img onMouseEnter={handleOpenInfo} onMouseLeave={handleOpenInfo} src='/img/questionMark-gray.png'></img>
    ...
    <div className="modal-content">
      {userInfo.bjId && <div className="connect-msg" ><span>백준 아이디가 연결되어 있습니다.</span></div>}
      {loadingMsg ==='BJCONNECT' && <LoadingSpinner loadingMsg={'연동 확인중입니다'} />}
      {!userInfo.bjId && <button className="eightbit-btn eightbit-btn--proceed" onClick={handleCodeRequest}>연동코드 받기</button>}
      {!userInfo.bjId && connectCode &&
      <div className="code-content">
        <h3>{connectCode}</h3>
        <button className="copy" onClick={()=>{handleCopyCode(connectCode)}}>코드복사하기</button>
      </div>}
      <form className="bjFrom" onSubmit={handleCheckBjConnect}>
        <label htmlFor="bjId">나의 백준아이디</label><br/>
        <input disabled={userInfo.bjId?true:false} type="text" name="bjId" id="bjId" value={bjIdForm} onChange={handleInputChange} />
        {!userInfo.bjId && <button className="eightbit-btn eightbit-btn--proceed" >연동하기</button>}
      </form>
    </div>
  </Wrapper>
}
