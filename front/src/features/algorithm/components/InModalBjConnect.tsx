...
function InModalBjConnect() {
  ...
  return <>
    ...
    {connectCode && <><h3>{connectCode}</h3> <button onClick={()=>{handleCopyCode(connectCode)}}>코드복사하기</button></>}
    <form onSubmit={handleCheckBjConnect}>
      <label htmlFor="bjId">나의 백준아이디</label>
      <input type="text" name="bjId" id="bjId" value={bjIdForm} onChange={handleInputChange} />
      <button>연동하기</button>
    </form>
  </>
}
