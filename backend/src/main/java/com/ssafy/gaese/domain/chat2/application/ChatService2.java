...
//        List<ChatDto> chatDtoList = chatRepository.findAllByFromIdAndToIdOrToIdAndFromId(from,to).stream().map(chat -> chat.toDto()).collect(Collectors.toList());
//        chatDtoList.addAll(chatRepository.findAllByFromIdAndToIdOrToIdAndFromId(to,from).stream().map(chat -> chat.toDto()).collect(Collectors.toList()));
//        return chatDtoList;
//    }
//
